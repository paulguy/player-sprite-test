	...
	match action:
		Action.STANDING:
			if grounded:
				...
				elif can_climb > 0 and pressed_vert < 0.0:
					# try to climb
					grab_ray.force_raycast_update()
					if grab_ray.is_colliding():
			...
			if grab_ray.is_colliding():
				if (facing_dir > 0.0 and Input.is_action_just_pressed(&"move_left")) or \
				   (facing_dir < 0.0 and Input.is_action_just_pressed(&"move_right")):
					...
					if (can_climb > 0 or
						(can_climb == 0 and pressed_vert != last_climb_dir)) and \
						pressed_vert != 0.0:
						climb_anim(facing_dir)
			else:
				last_climb_dir = 0.0
		...
		Action.CLIMBING:
			...
			if grab_ray.is_colliding():
				...
				if pressed_vert > 0.0 and is_on_floor():
					...
					velocity.y = 0.0
					if can_climb > 0 or \
					   (can_climb == 0 and pressed_vert != last_climb_dir):
		...
		Action.SWIMMING:
			if in_water == 0:
				...
				if grounded:
					...
					if can_climb > 0:
						grab_ray.force_raycast_update()
					if can_climb > 0 and grab_ray.is_colliding():
						# grab wall
						# move against wall
						velocity.y = 0.0
						move_and_collide(Vector2(grab_ray.target_position.x, 0.0))
